...
from gru_only_core import RatioSplitConfig, TARGET_KEYS
from hybrid_core import TrainConfig, save_json
from lstm_only_core import LSTMConfig, benchmark_single_dataset, resolve_datasets
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Clean LSTM-only benchmark for AMZN/JPM/TSLA with chronological 7/3 or 7/2/1 splits."
    )
    ...
    parser.add_argument("--fast", action="store_true", help="Use only the first two LSTM configs when validation exists.")
    parser.add_argument("--hidden-dim", type=int, default=None, help="Force a single LSTM config instead of tuning.")
    parser.add_argument("--num-layers", type=int, default=1)
    parser.add_argument("--dropout", type=float, default=0.10)
    parser.add_argument("--lr", type=float, default=1e-3)
    parser.add_argument("--output-dir", type=str, default="./lstm_benchmark_outputs")
...
def main() -> None:
    ...
    for dataset_name, path in datasets:
        ...
        report_lines = [
            f"# LSTM-only benchmark: {dataset_name}",
            "",
            "## Protocol",
