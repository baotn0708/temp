...
from lstm_only_core import LSTMConfig, fit_final_lstm, save_training_artifacts, select_lstm_config
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train final LSTM-only models and evaluate reconstructed next-day OHLC prices."
    )
    ...
    parser.add_argument("--output-dir", type=str, default="./lstm_train_outputs")
...
def main() -> None:
    ...
    for dataset_name, path in datasets:
        ...
        lstm_cfg, selected_epoch, val_metrics = select_lstm_config(
            split=split,
            train_cfg=train_cfg,
        ...
        model, scaler, test_metrics = fit_final_lstm(
            split=split,
            lstm_cfg=lstm_cfg,
            train_cfg=train_cfg,
